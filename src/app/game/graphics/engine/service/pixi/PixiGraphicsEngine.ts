...
export class PixiGraphicsEngine extends GraphicsEngineBase {
  ...
  private readonly textureMetadataMap: Map<string, TextureMatadata>;
  ...
  createText(aid: string, text: string | number, textConfig: TextConfig, animation?: Animation): void {
    ...
    if (textConfig.centerRangeX) {
      const avg = MathService.getAverageValue(textConfig.centerRangeX.min, textConfig.centerRangeX.max);
      textObj.x = avg - MathService.round(textObj.width / 2);
    } else {
      textObj.x = textConfig.coordinates.x;
    }

    if (textConfig.centerRangeY) {
      const avg = MathService.getAverageValue(textConfig.centerRangeY.min, textConfig.centerRangeY.max);
      textObj.y = avg - MathService.round(textObj.height / 2);
    } else {
  ...
  private handleSpritePosition(
    ...
    const ratio = MathService.round(textureSize.width / textureSize.height, 2);
    const height = rect.dimensions.height;
    const width = MathService.round(height * ratio);
    const offset = MathService.round(Math.abs(width - rect.dimensions.width) / 2);
